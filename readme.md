> [!WARNING]  
> We've observed inconsistent behavior in the Azure Durable Functions state management due to a combination of high frequency events from \<ConversationRelay\> and the way Durable Functions leverages queues and tables for state management.  It also lacks support for proper CancellationToken support which would allow us to cancel long running tasks, which is critical for handling interrupts properly.  This forced us to build in unnatural patterns which likely compounded the state management problem.  We're actively looking to migrate away from **Azure Durable Functions** into **App Container Service**.  You can get a simple demo up and working quickly with this pattern, but we cannot recommend it for production use.
